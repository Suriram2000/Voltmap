...
import 'package:voltmap/features/discovery/presentation/charger_details_hero.dart';
...
void main() {
  ...
  testWidgets('Android production search, details, and map screenshots',
    ...
    final photo = find.byKey(const Key('chargerDetailsPhoto'));
    await waitFor(tester, photo);
    await tester.runAsync(() async {
      await precacheImage(
        const AssetImage(ChargerDetailsHero.assetPath),
        tester.element(photo),
      ).timeout(const Duration(seconds: 30));
    });
    await tester.pump(const Duration(seconds: 2));
    final renderedPhoto = find.descendant(
      of: photo,
      matching: find.byType(RawImage),
    );
    expect(tester.widget<RawImage>(renderedPhoto).image, isNotNull);
